...
export default function App() {
  ...
  async function fetchCalls() {
    ...
    try {
      ...
      while (hasMore) {
        ...
        // Stop if we got less than 50 (last page) or no meta
      ...
      if (!allCalls.length) showError("No calls found for these filters. Try a wider date range.");
      setCalls(allCalls);
  ...
  function reset() {
    setStep(0); setCalls([]); setTranscripts({});
    setSelectedCalls({}); setError(""); setDownloadProgress(0);
  }
    ...
    <div style={{padding:"1.5rem 1rem",maxWidth:720,margin:"0 auto",fontFamily:"system-ui,sans-serif",color:"#111"}}>
      ...
      {step===0&&(
        <Card>
          ...
          {loadingCalls&&<p style={{fontSize:12,color:"#666",marginTop:8}}>Fetching all pages of calls, please wait…</p>}
        </Card>
      )}
