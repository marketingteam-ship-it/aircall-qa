...
export default function App() {
  ...
  const [activeTab,setActiveTab] = useState("QA Audit");
  ...
  async function fetchCustomerCalls() {
    ...
    setCustomerLoading(true); setCustomerCalls([]); setCustomerTranscripts({}); setCustomerError("");
    try {
  ...
  function downloadAsText() {
    ...
    a.href=url; a.download=`transcripts-${customerPhone.replace(/\D/g,"")}.txt`;
...
Transcript:\n${text.slice(0,1200)}`;
  ...
  return (
    <div style={{padding:"1.5rem 1rem",maxWidth:720,margin:"0 auto",fontFamily:"system-ui,sans-serif",color:"#111"}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                ...
                <div style={{display:"flex",gap:8}}>
                  ...
                  {Object.keys(customerTranscripts).length>0&&(
                    <Btn onClick={downloadAsText} variant="success">⬇ Download .txt</Btn>
                  )}
                </div>
              </div>
                  ...
                  return (
                    <div key={c.id} style={{borderRadius:8,border:"1px solid #e5e5e5",overflow:"hidden"}}>
                      <div style={{display:"flex",alignItems:"center",gap:12,padding:"10px 12px",background:"#fafafa"}}>
                        ...
                        {transcript&&<Badge color="green">Transcript ready</Badge>}
                      </div>
                      {transcript&&(
      ...
      {step===1&&(
        <Card>
          <h3 style={{fontSize:15,fontWeight:500,marginBottom:16}}>Step 2 — Date range, call type & agent</h3>
          ...
          {users.length>0&&(
            <div style={{marginBottom:20}}>
              <label style={{fontSize:12,color:"#666",display:"block",marginBottom:6}}>Filter by agent (optional)</label>
